...
public class o {
    ...
    public static class a {
        ...
        /* renamed from: c, reason: collision with root package name */
        public static final int f1058c = 4099;
    ...
    private String m() {
        ...
        try {
            ...
            if (TextUtils.isEmpty(str)) {
                ...
                if (this.f1057c == null) {
                    this.f1057c = PreferenceWrapper.getDefault(a);
                }
                String string = this.f1057c.getString("pre_version", "");
                String appVersionName = DeviceConfig.getAppVersionName(a);
                if (TextUtils.isEmpty(string)) {
                    this.f1057c.edit().putString("pre_version", "0").putString("cur_version", appVersionName).commit();
                    str = "0";
                } else {
                    String string2 = this.f1057c.getString("cur_version", "");
                    if (appVersionName.equals(string2)) {
                        str = string;
                    } else {
                        this.f1057c.edit().putString("pre_version", string2).putString("cur_version", appVersionName).commit();
                        str = string2;
                    }
                }
            }
    ...
    private String n() {
        ...
        try {
            ...
            if (TextUtils.isEmpty(str)) {
                ...
                String string = this.f1057c.getString("pre_date", "");
                if (TextUtils.isEmpty(string)) {
                    string = new SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(new Date(System.currentTimeMillis()));
                    this.f1057c.edit().putString("pre_date", string).commit();
                } else {
                    String format = new SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(new Date(System.currentTimeMillis()));
                    if (!string.equals(format)) {
                        this.f1057c.edit().putString("pre_date", format).commit();
                        str = format;
                    }
                }
                str = string;
